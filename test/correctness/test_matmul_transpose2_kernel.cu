...
int main() {
  ...
  constexpr int N = 4096;  // A rows / C rows
  constexpr int K = 1024;  // A cols / B cols
  constexpr int M = 4096;  // B rows / C cols
  ...
  // B is stored as M x K so kernel computes A (N x K) * B^T (K x M).
  const size_t sizeA = static_cast<size_t>(N) * K;
  const size_t sizeB = static_cast<size_t>(M) * K;
  ...
  std::cout << "Launching matmul_transpose2 with requested parameters..." << std::endl;
  matmul_transpose2<BN, BK, BM, RN, RM><<<blocks, threads_per_block>>>(dA, dB, dC, N, M, K);
  ...
  // CPU reference: R = A * B^T (row-major), using utils/MatricesOp.cpp
  Multiply_transpose2(hA_from_device.data(), hB_from_device.data(), hC_cpu.data(), N, K, M);
