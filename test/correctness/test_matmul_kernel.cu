...
int run_case(int rows, int inner, int cols, std::mt19937& random) {
  const size_t A_size = static_cast<size_t>(rows) * inner;
  ...
  std::vector<float> bias(cols, 0.0f);
  ...
  Multiply(A.data(), B.data(), expected.data(), rows, inner, cols);
  ...
  CUDA_CHECK(cudaMalloc(&device_bias, cols * sizeof(float)));
