...
int main() {
  ...
  constexpr int K = 1024;  // A cols / B rows
  ...
  std::cout << "Launching matmul with requested parameters..." << std::endl;
  matmul<BN, BK, BM, RN, RM><<<blocks, threads_per_block>>>(dA, dB, dC, N, M, K);
  ...
  // CPU reference: R = A * B (row-major), using utils/MatricesOp.cpp
  Multiply(hA_from_device.data(), hB_from_device.data(), hC_cpu.data(), N, K, M);
